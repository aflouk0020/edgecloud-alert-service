...
class AlertEscalationSchedulerTest {
 @Test void evaluatesBoundedBatchAndIsolatesPerAlertFailure(){AlertEventRepository alerts=mock(AlertEventRepository.class);AlertEscalationProcessor processor=mock(AlertEscalationProcessor.class);UUID first=UUID.randomUUID(),second=UUID.randomUUID();when(alerts.findActiveIds(any(Pageable.class))).thenReturn(List.of(first,second));when(processor.process(eq(first),any())).thenThrow(new RuntimeException("isolated"));AlertEscalationScheduler scheduler=new AlertEscalationScheduler(alerts,processor,25);scheduler.evaluate();verify(processor).process(eq(first),any());verify(processor).process(eq(second),any());}
}
