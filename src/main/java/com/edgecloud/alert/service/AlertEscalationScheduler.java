...
public class AlertEscalationScheduler {private static final Logger log=LoggerFactory.getLogger(AlertEscalationScheduler.class);private final AlertEventRepository alerts;private final AlertEscalationProcessor processor;private final int batch;
 ...
 @Scheduled(fixedDelayString="${alert.escalation.poll-interval-ms:30000}") public void evaluate(){int count=0;for(var id:alerts.findActiveIds(PageRequest.of(0,batch))){try{count+=processor.process(id,Instant.now());}catch(RuntimeException ex){log.warn("Escalation evaluation failed for alert {}",id);}}if(count>0)log.info("Processed {} alert escalation levels",count);}
}
